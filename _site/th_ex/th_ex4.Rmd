...
In this take-home exercise 4, we are going to analyse and desribe the **daily routines** of two selected participants of the city of Engagement, Ohio USA by using appropriate visual analytics methods.
...
The packages required for this exercise are [**tidyverse**](https://www.tidyverse.org/), [**data.table**](https://rdatatable.gitlab.io/data.table/index.html), [**ViSiElse**](https://cran.r-project.org/web/packages/ViSiElse/index.html) and [**zoo**](https://cran.r-project.org/web/packages/zoo/zoo.pdf) and patchwork.
...
We will randomly select 2 participants from the original dataset, namely Participant #1 and #2 by using `filter()` or dplyr and we will only see how typical Monday and Sunday look like for each participant by selecting May 2, 2022 and May 8, 2022. To do this, we will create new column called *Date* by using `as.Date()` of zoo.
...
The initial code chunk is shown here.

```{r}
logs1_v <- logs %>%
  filter(participantId == '1') %>%
  unite(Activity, c(currentMode,sleepStatus), sep = '-', remove = FALSE) %>%
  mutate(EndTime = lead(timestamp)) %>%
  mutate(Date=as.Date(timestamp),
         EndTime = lead(timestamp),
         Day = weekdays(as.Date(timestamp)),
